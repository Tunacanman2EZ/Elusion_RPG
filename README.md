...
That serves nineteen endpoints — accounts and sessions, character saves, the shared bank, combat kills and loot — plus interactive Swagger docs (via flasgger) at `http://127.0.0.1:5000/apidocs`, which is the quickest way to see the whole surface at once.
...
Active work is moving authority off the client. The server already rolls every loot drop with entropy the client never sees, owns level and XP and the stat maxima they imply, and holds loot bags as rows the game renders a copy of — taking an item out of one is a request, not an announcement. The remaining gap is the backpack ledger, which is still whatever the client pushes on save.
