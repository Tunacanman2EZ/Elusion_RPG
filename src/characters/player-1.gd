...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed:
		match event.keycode:
			...
			# PET ITEM GRANTS — T, Y, K.
			#
			# These grant the pet ITEM rather than spawning the pet directly,
			# so they exercise the real path: inventory -> use -> summon_pet()
			# -> active_pet_id -> survives a scene change. The U/I/O/P keys
			# below skip all of that and drop a bare node into the scene,
			# which is why they never caught that the pet items didn't exist.
			# Prefer these three when testing anything about pets.
			#
			# The two slimes have no direct-spawn key on purpose. Adding one
			# would mean two more _debug_spawn_* functions duplicating what
			# summon_pet() already does properly.
			#
			# NOT F8: that's the editor's "Stop running project" shortcut and
			# it kills the game even when the game window has focus.
			KEY_T: _debug_give_item("petpoisonslimesmall", 1)
			KEY_Y: _debug_give_item("petpoisonslimelarge", 1)
			# moved off Y to make room for the slimes above. K is unbound in
			# the input map and in every other script.
			KEY_K: _debug_give_item("petsniper", 1)
