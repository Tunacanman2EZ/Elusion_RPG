## A grid-based inventory container that manages InventorySlot nodes.
...
func _create_slots() -> void:
	...
	for i in range(capacity):
		var slot_instance = inventory_slot_scene.instantiate() as InventorySlotUI
...
func get_slot_at(index: int) -> InventorySlotUI:
	if index < 0 or index >= slots.size():
		return null
	return slots[index]
...
func _on_slot_right_clicked(slot: InventorySlotUI) -> void:
	slot_right_clicked.emit(slot)


func _on_slot_hovered(slot: InventorySlotUI) -> void:
	slot_hovered.emit(slot)


func _on_slot_unhovered(slot: InventorySlotUI) -> void:
