## Represents a single slot in the inventory grid.[br]
## Handles item display, hover/selection states, and user interactions.
...
## Creates the slot styles - uses theme if available, otherwise creates programmatically.
func _create_styles() -> void:
	# Try to get styles from the theme first
	...
	# Fallback to programmatic styles if theme styles weren't found
...
## Sets the item in this slot and refreshes the display.[br]
## Updates the item's slot_index to match this slot.
...
## Clears the item from this slot, leaving it empty.
...
## Refreshes the visual display of the slot.[br]
## Updates the icon texture and quantity label based on current item.
...
## Sets the visual hover state of the slot.
...
## Updates the panel style based on current state.
...
## Handles input events on the slot.
...
## Handles mouse entering the slot area.
func _on_mouse_entered() -> void:
	set_hovered(true)
	slot_hovered.emit(self)


## Handles mouse exiting the slot area.
